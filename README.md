# Hi, I'm Nikita 👋

Full-Stack Developer based in Tel Aviv with 4 years of experience building scalable web applications, backend services, and automation tools. Passionate about solving complex problems and turning ideas into reliable production systems.

## 🛠 Tech Stack

**Languages:** JavaScript, TypeScript, Python  

**Frontend:** React.js, Chakra UI, Tailwind CSS, HTML, CSS  

**Backend:** Node.js, NestJS, Python, REST APIs  

**Databases:** PostgreSQL, MySQL, MongoDB  

**Cloud & DevOps:** Docker, Kubernetes, AWS, Azure, CI/CD  

**Testing:** Jest, Pytest  

**Tools:** GitHub Actions, GitLab, Jira  

## 🚀 Projects

**[Mr. Donald's Burger Shop](https://github.com/nikitaLevin/mr-donalds-burger-shop)** — Food ordering React app with cart, toppings and item customization

**[3Glo Landing](https://github.com/nikitaLevin/3dglo-landing)** — Landing page for a 3D modeling studio with cost calculator and portfolio carousel

**[Form Validator](https://github.com/nikitaLevin/form-validator)** — Reusable JS validator plugin with email and phone validation

**[Todo App](https://github.com/nikitaLevin/todo-app)** — Simple task manager with active and completed task lists

**[MCP Server Creation Tool](https://github.com/nikitaLevin/mcp-server-creation-tool)** — Python tool that automatically scaffolds new MCP servers

**[My Portfolio](https://github.com/nikitaLevin/my-portfolio)** — Personal portfolio website built with React and Chakra UI

## 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=nikitaLevin&show_icons=true)

## 💼 Open to Work

Currently looking for **Full-Stack**, **Backend**, or **Frontend** developer roles in **Israel (Tel Aviv area)**.

## 🔗 Contact

[![Email](https://img.shields.io/badge/Email-levin.nikitaa@gmail.com-blue?style=flat&logo=gmail)](mailto:levin.nikitaa@gmail.com)

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Nikita_Levin-blue?style=flat&logo=linkedin)](https://www.linkedin.com/in/hi-tech-nikita-levin/)

[![Telegram](https://img.shields.io/badge/Telegram-@hi__tech__levin-blue?style=flat&logo=telegram)](https://t.me/hi_tech_levin)
